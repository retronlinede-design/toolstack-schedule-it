    ...
    <div className="space-y-8">
      ...
      {driverGroups.map((group) => (
        <section key={group.driverId} className="min-w-0 max-w-full rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-3 lg:hidden print:hidden">{group.days.map((summary) => <article key={`card-${summary.driverId}-${summary.date}`} className="ts-card p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-semibold">{formatLongDate(summary.date)}</h4>{summary.overnight ? <Badge tone="info">Overnight</Badge> : null}</div><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-neutral-500">Duty</dt><dd>{summary.startTime}–{summary.endTime}</dd></div><div><dt className="text-neutral-500">Total span</dt><dd>{minutesToDuration(summary.totalSpanMinutes)}</dd></div><div><dt className="text-neutral-500">Counted work</dt><dd>{summary.totalDuration}</dd></div><div><dt className="text-neutral-500">Overtime</dt><dd>{summary.overtimeDuration}</dd></div><div><dt className="text-neutral-500">Active / travel</dt><dd>{minutesToDuration(summary.activeMinutes)} / {minutesToDuration(summary.travelMinutes)}</dd></div><div><dt className="text-neutral-500">Standby / breaks</dt><dd>{minutesToDuration(summary.standbyMinutes)} / {minutesToDuration(summary.effectiveBreakMinutes)}</dd></div><div><dt className="text-neutral-500">Duty segments</dt><dd>{summary.dutySegmentCount}</dd></div><div><dt className="text-neutral-500">Rest before next</dt><dd>{summary.restBeforeNextDutyMinutes == null ? "-" : minutesToDuration(summary.restBeforeNextDutyMinutes)}</dd></div></dl>{summary.pickupCount ? <p className="mt-3 text-xs font-semibold">Includes {summary.pickupCount} pre-departure pickup{summary.pickupCount === 1 ? "" : "s"}{summary.firstPickupTime ? ` · First at ${summary.firstPickupTime}` : ""}</p> : null}<div className="mt-3"><Notes notes={summary.notes} /></div></article>)}</div>
              ...
              <tbody>
                {group.days.map((summary) => (
                  <tr key={`${summary.driverId}-${summary.date}`} className={`border-t border-neutral-100 ${summary.shortRest ? "bg-red-50/40" : ""}`}>
                    ...
                    <td className="border border-neutral-200 p-3">
                      <div className="flex flex-wrap gap-1"><Notes notes={summary.notes} /><Badge>{movementMeta(summary.driverId, summary.date).count} movements</Badge>{summary.pickupCount ? <Badge tone="info">{summary.pickupCount} pickups{summary.firstPickupTime ? ` · ${summary.firstPickupTime}` : ""}</Badge> : null}{movementMeta(summary.driverId, summary.date).overnight ? <Badge tone="info">Overnight</Badge> : null}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
