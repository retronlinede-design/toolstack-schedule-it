...
export default function PreviewWorkspace({ tabs, selectedView, onViewChange, scheduleDays, selectedDriverName, documentTitle, srcDoc, frameRef, onPrint, onCopy, onClose }) {
  ...
  return <ModalShell title="Document Preview" subtitle={`${selected?.label || documentTitle} · ${dateRange(scheduleDays)}`} onClose={onClose}>
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div className="text-sm text-[var(--ts-text-muted)]">{selectedView === "driver" && selectedDriverName ? `Driver: ${selectedDriverName}` : selected?.label}</div>
      <div className="flex flex-wrap gap-2"><Button onClick={runCopy}><Clipboard className="h-4 w-4" /> Copy programme</Button><Button onClick={onPrint} variant="primary"><Printer className="h-4 w-4" /> Print / Save PDF</Button></div>
    </div>
