...
export function buildHtmlImportCandidate(current, result, mode) {
  ...
  const importedMovements = result.movements.map((movement, index) => {
    ...
    return { ...fields, audiences: fields.audiences || { ...DEFAULT_MOVEMENT_AUDIENCES, driverIds: [] }, continuesOvernight: false, conflictOverrides: [], id: `html-movement-${fingerprint}-${index + 1}`, scheduleDayId: targetDay.id, sortOrder: (index + 1) * 10, driverId: driver?.id || "", vehicleId: vehicle?.id || "" };
  });
  const candidate = {
    ...current,
    drivers,
