...
export function getExportBodyHtml(schedule, view, options = {}) {
  return bodyForView(schedule, view, options.selectedDriverId);
}
...
export function getExportDocument(schedule, view, options = {}) {
  ...
  const metaHtml = isExecutive
    ? `
          <span class="document-meta"><span class="meta-label">Document:</span> ${escapeHtml(headerTitle(schedule, view, options.selectedDriverId))}</span>
          <span class="date-meta"><span class="meta-label">Date:</span> ${escapeHtml(dateLabel(schedule))}</span>
        `
    : `
          <span class="date-meta">${escapeHtml(dateLabel(schedule))}</span>
          <span class="subtitle document-meta">${escapeHtml(title)}</span>
  ...
  const tableHtml = getExportBodyHtml(schedule, view, options);
